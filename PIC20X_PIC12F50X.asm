;*******************************************************************************
 TITLE      PIC20X+PIC12F50X+RDA5807M+IR.asm
...
frequency_h ;EQU    0018h    ;Frequency high byte
;********************************************************************
...
;    MOVLW  0x0F           ;Save 0x3F into 'port' register
	MOVLW  0x3F           ;Save 0x3F into 'port' register
...
; movlw 0x40 ;91.90 MHz
 movlw freqL ;91.90 MHz
 movwf frequency_l
; movlw 0x0C ;91.90 MHz
...
;   MOVLW  0xD0           ;TEST added to enable BASS 
    CALL   I2C_WRITE_BYTE
...
CH_UP:
    ...
    CALL   I2C_WRITE_BYTE ;And write it via I2C
    CALL   I2C_STOP       ;Issue I2C Stop condition
    GOTO   _LOOP           ;And return to the 'LOOP' labe;
...
;##############################################################################
;---IR REMOTE CONTROL 38kHz ROUTINE----------------/[NEC]\---------------------
;##############################################################################
